...
if [[ ${WITH_NETDATA:-false} == true ]]; then
    ...
    git checkout ./netdata/configs >/dev/null 2>&1 || true

    printf "\n\033[3;33m[DEPLOY] Criando novo container NETDATA para manipular os dados dos novos containers APP/PHP-FPM && Webserver/NGINX 🐳 \033[0m\n\n"
    ...
    make docker/service/up context=netdata
fi
